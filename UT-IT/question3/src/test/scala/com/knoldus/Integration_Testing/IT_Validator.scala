...
class IT_Validator extends AsyncFlatSpec {
  ...
  "Email Id " should "be Valid" in {
    val result = companyImpl.getEmailId("meenakshi@gmail.net")
    assert(result == "valid")
  }

  "Email Id with wrong Recipient name " should "be invalid" in {
    val result = companyImpl.getEmailId("!!meenakshi@gmail.com")
  ...
  " A email " should "be Invalid without @ symbol" in {
    val result = userImpl.getEmailId("meenakshi.com")
    assert(result == "invalid")
  }

  "A email " should "be Invalid wrong domain name" in {
    val result = userImpl.getEmailId("meenakshi@gag!.com")
    assert(result == "invalid")
  }
  ...
  "A email " should "Invalid with wrong Top-Level Domain Name " in {
    val result = emailImpl.getEmailId("meenakshi@gmail.ccom")
    assert(result=="no")
  }
}
